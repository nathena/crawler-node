...
var request = exports.request = function request(url,worker){
    ...
    var promise = new Promise(function(resolve,reject){
        ...
        var _uri = parserUrl.parse(url);
        _uri = _uri["protocol"]+"//"+_uri["hostname"];
        ...
        if( worker["options"] && worker["options"]["jar"] ){

            if (Array.isArray(worker["options"]["jar"])) {
                worker["options"]["jar"].forEach(jar)
            } else {
                jar(worker["options"]["jar"])
            }
            function jar(cookie){
                if( cookie ){
                    cookiejar.setCookie(cookie,_uri,{http:true,ignoreError:true});
                }
            }
        ...
        req(options,function(err,res,body){
            ...
            if( res.headers["set-cookie"] ){
                worker["options"]["jar"] = res.headers["set-cookie"];
            }
